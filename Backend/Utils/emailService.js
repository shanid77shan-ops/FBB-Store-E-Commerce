...
class EmailService {
  ...
  canSendEmail() {
    this.checkMonthlyLimit();
    return this.emailEnabled && this.sentCount < this.monthlyLimit;
  }
  ...
  async sendOtpEmail(email, otp) {
    if (!this.canSendEmail()) {
      console.warn('Email service disabled or monthly limit reached');
      return false;
    }
    ...
    try {
      ...
      this.incrementCount();
      console.log('OTP email sent successfully. Monthly count:', this.sentCount);
      return true;
    } catch (error) {
  ...
  async sendOrderConfirmationEmail(email, order) {
    ...
    try {
      ...
      this.incrementCount();
      console.log('Order confirmation email sent successfully. Monthly count:', this.sentCount);
      return true;
    } catch (error) {
  ...
  async sendSellerNewOrderEmail(sellerEmail, order, sellerOrder) {
    if (!this.canSendEmail()) {
      console.warn('Email service disabled or monthly limit reached');
      return false;
    }
    ...
    try {
      ...
      this.incrementCount();
      console.log('Seller new order email sent successfully. Monthly count:', this.sentCount);
      return true;
    } catch (error) {
  ...
  async sendPasswordResetEmail(email, resetToken) {
    ...
    try {
      ...
      this.incrementCount();
      console.log('Password reset email sent successfully. Monthly count:', this.sentCount);
      return true;
    } catch (error) {
  ...
  async sendOrderStatusUpdateEmail(email, order, sellerOrder) {
    if (!this.canSendEmail()) {
      console.warn('Email service disabled or monthly limit reached');
      return false;
    }
    ...
    try {
      ...
      const sellerName = sellerOrder.seller?.name || 'Seller';
      const sellerItems = sellerOrder.items || [];
      
      switch (sellerOrder.sellerStatus) {
        case 'shipped':
          subject = `Part of Your Order #${order.orderId} Has Been Shipped!`;
          statusText = 'Shipped';
          message = `${sellerName} has shipped items from your order.`;
          break;
        case 'delivered':
          subject = `Part of Your Order #${order.orderId} Has Been Delivered!`;
          statusText = 'Delivered';
          message = `${sellerName} has delivered items from your order.`;
          break;
        case 'cancelled':
          subject = `Items from Order #${order.orderId} Cancelled`;
          statusText = 'Cancelled';
          message = `${sellerName} has cancelled items from your order.`;
          break;
        default:
          subject = `Order #${order.orderId} Status Update`;
          statusText = 'Updated';
          message = `${sellerName} has updated status for items in your order.`;
      }
        ...
        <tr style="border-bottom: 1px solid #eee;">
          ...
          <td style="padding: 12px; text-align: center; vertical-align: top;">
            <span style="display: inline-block; 
                      background: ${item.itemStatus === 'shipped' ? '#007bff' : item.itemStatus === 'delivered' ? '#28a745' : item.itemStatus === 'cancelled' ? '#dc3545' : '#ffc107'}; 
                      color: white; 
                      padding: 4px 10px; 
                      border-radius: 12px; 
                      font-size: 12px; 
          ...
          <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 30px; background: #ffffff; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
            ...
            <div style="background: #f9f9f9; padding: 25px; border-radius: 10px; margin-bottom: 30px; text-align: center;">
              ...
              <div style="font-size: 48px; margin: 20px 0;">
                ${sellerOrder.sellerStatus === 'shipped' ? '🚚' : sellerOrder.sellerStatus === 'delivered' ? '✓' : sellerOrder.sellerStatus === 'cancelled' ? '✗' : '📦'}
              </div>
              <p style="color: #555; font-size: 18px; margin-bottom: 10px;">
                Order #${order.orderId}
              </p>
              <div style="display: inline-block; 
                        background: ${sellerOrder.sellerStatus === 'shipped' ? '#007bff' : sellerOrder.sellerStatus === 'delivered' ? '#28a745' : sellerOrder.sellerStatus === 'cancelled' ? '#dc3545' : '#ffc107'}; 
                        color: white; 
                        padding: 8px 20px; 
                        border-radius: 20px; 
                        font-weight: 500;">
                ${statusText} by ${sellerName}
              </div>
            </div>
            ...
            <div style="margin-bottom: 30px;">
              <h3 style="color: #333; margin-bottom: 15px;">Updated Items</h3>
              <table style="width: 100%; border-collapse: collapse;">
                <thead>
                  <tr style="background: #f8f9fa;">
                    ...
                    <th style="padding: 12px; text-align: right; font-weight: 600; color: #333; font-size: 14px;">Price</th>
                    <th style="padding: 12px; text-align: right; font-weight: 600; color: #333; font-size: 14px;">Total</th>
                    <th style="padding: 12px; text-align: center; font-weight: 600; color: #333; font-size: 14px;">Status</th>
                  </tr>
                </thead>
                <tbody>
                  ${itemsHtml}
            ...
            <div style="background: #f9f9f9; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
              <h3 style="color: #333; margin-bottom: 15px;">Seller Summary</h3>
              <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                <span style="color: #666;">Seller:</span>
                <span style="font-weight: 500;">${sellerName}</span>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                <span style="color: #666;">Items Updated:</span>
                <span style="font-weight: 500;">${sellerItems.length}</span>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                <span style="color: #666;">Seller Status:</span>
                <span style="font-weight: 500; color: ${sellerOrder.sellerStatus === 'shipped' ? '#007bff' : sellerOrder.sellerStatus === 'delivered' ? '#28a745' : sellerOrder.sellerStatus === 'cancelled' ? '#dc3545' : '#ffc107'}">
                  ${sellerOrder.sellerStatus}
                </span>
              </div>
              ${sellerOrder.sellerNotes ? `
                <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #ddd;">
                  <div style="color: #666; margin-bottom: 5px;">Seller Notes:</div>
                  <div style="color: #333; font-style: italic;">${sellerOrder.sellerNotes}</div>
                </div>
              ` : ''}
            </div>

            ${sellerItems.some(item => item.trackingNumber) ? `
              <div style="margin-bottom: 30px;">
                <h3 style="color: #333; margin-bottom: 15px;">Tracking Information</h3>
                <div style="background: #f0f7ff; padding: 20px; border-radius: 8px;">
                  ${sellerItems.filter(item => item.trackingNumber).map(item => `
                    <div style="margin-bottom: 15px;">
                      ...
                      <p style="color: #666; margin-bottom: 5px;">
                        <strong>Carrier:</strong> ${item.shippingProvider || 'Standard Shipping'}
                      </p>
                      ${item.estimatedDeliveryDate ? `
                        <p style="color: #666;">
                          <strong>Estimated Delivery:</strong> ${new Date(item.estimatedDeliveryDate).toLocaleDateString()}
                        </p>
                      ` : ''}
                    </div>
                  `).join('')}
            ...
            <div style="margin-bottom: 30px;">
              <h3 style="color: #333; margin-bottom: 15px;">Your Order Summary</h3>
              <div style="background: #f9f9f9; padding: 20px; border-radius: 8px;">
                <p style="color: #333; margin-bottom: 5px;">
                ...
                <p style="color: #333; margin-bottom: 5px;">
                  ...
                  <span style="color: ${order.status === 'delivered' ? '#28a745' : order.status === 'cancelled' ? '#dc3545' : order.status === 'shipped' ? '#007bff' : '#ffc107'}; 
                         font-weight: 500;">
                    ${order.status}
                  </span>
                </p>
      ...
      this.incrementCount();
      console.log('Order status update email sent successfully. Monthly count:', this.sentCount);
      return true;
    } catch (error) {
